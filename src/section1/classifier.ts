...
import type { LoadedSource } from '../loaders/sources.js';

export interface StablecoinClassificationInput {
  assetName: string;
  issuerName: string;
  sources: LoadedSource[];
  tone?: 'sharp' | 'descriptive';
}
...
function buildSectionIntro(tone: 'sharp' | 'descriptive' = 'descriptive'): string {
  if (tone === 'sharp') {
...
function buildSourceBundle(sources: LoadedSource[], maxChars = 20_000): string {
...
function buildClassificationPrompt(input: StablecoinClassificationInput): string {
...
export async function runSection1Classification(input: StablecoinClassificationInput): Promise<ClassificationResult> {
