...
exports.classifyFromSignals = classifyFromSignals;
function classifyFromSignals(signals) {
    ...
    if ((signals.mentionsCash || signals.mentionsTBills || signals.mentionsRepo || signals.mentionsMMF) && signals.mentionsCustodian) {
        const parts = [];
        if (signals.mentionsTBills)
            parts.push("T-bills");
        if (signals.mentionsCash)
            parts.push("cash");
        if (signals.mentionsRepo)
            parts.push("repo");
        if (signals.mentionsMMF)
            parts.push("MMF");
    ...
    const fallback = "Crypto-Backed Hybrid Collateral";
    return {
        type: fallback,
        confidence: 0.2,
